...
def obtener_tratamiento( riesgo ):
    try:
        ...
        # Fallback simple cuando no hay acceso al modelo local
        return 'Aplicar control de acceso y cifrado, junto con política de contraseñas robusta.'
...
• **Fallos hardware**: daño debido a causas como sobrecalentamiento, sobrecarga eléctrica o errores en la manufactura, lo que lleva a una pérdida de datos o inoperatividad del sistema.""" },
        ...
        # Si el modelo no responde en el formato esperado, crear un fallback simple
        if len(riesgos) == 0:
            raise ValueError('Formato de respuesta inesperado')

        return riesgos, impactos
    except Exception as e:
        # Fallback estático cuando no hay acceso al modelo local o hay error en parseo
        fallback = [
            ('Acceso no autorizado', f'Acceso no autorizado a {activo} y posible divulgación de información'),
            ('Pérdida de datos', f'Pérdida de datos críticos relacionados con {activo}'),
            ('Vulnerabilidades de software', f'Vulnerabilidades explotables en software asociado a {activo}'),
            ('Interrupción del servicio', f'Disponibilidad afectada para {activo} por fallos o ataques'),
            ('Fuga de información', f'Divulgación no autorizada de información manejada por {activo}')
        ]
        riesgos = [r[0] for r in fallback]
        impactos = [r[1] for r in fallback]
        return riesgos, impactos
